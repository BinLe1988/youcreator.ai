	...
	api := r.Group("/api/v1")
	{
		...
		ai := api.Group("/ai")
		{
			ai.POST("/text/generate", func(c *gin.Context) {
				...
				log.Printf("Received text generation request: %+v", req)

				// 调用AI服务
				aiServiceURL := "http://localhost:8000"
				resp, err := http.Post(aiServiceURL+"/generate/text", "application/json", nil)
				if err != nil {
					log.Printf("AI service error: %v, using mock data", err)
					// 返回模拟数据作为备用
					mockText := generateMockText(req)
					c.JSON(http.StatusOK, gin.H{
						"success": true,
						"data": gin.H{
							"text": mockText,
							"metadata": gin.H{
								"word_count": len(mockText),
